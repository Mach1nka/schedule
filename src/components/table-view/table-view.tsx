...
const TableView: React.FC<any> = () => {
  ...
  />,
    ...
    },
    {
      ...
      dataIndex: 'status',
      render: (tag) => { 
        const textColor = setting && setting.[tag[0]] ? setting.[tag[0]].backgroundColor : currentTypes.find((type) => type.name === tag[0])?.color
        const color = setting && setting.[tag[0]] ? setting.[tag[0]].color : 'black'
        return (
